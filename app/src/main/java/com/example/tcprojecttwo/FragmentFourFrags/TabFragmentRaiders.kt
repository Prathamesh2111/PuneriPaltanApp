...
class TabFragmentRaiders : Fragment() {

    lateinit var playersAdapter: PlayersAdapter
    private var _binding: FragmentTabRaidersBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentTabRaidersBinding.inflate(inflater, container, false)

        binding.raidersRv.setHasFixedSize(false)
        linearLayoutManager = LinearLayoutManager(context)
        binding.raidersRv.layoutManager = linearLayoutManager
    ...
    fun getPlayersData() {
        val retrofitData = RetrofitInstance.apiInterface.getPlayers(1.toString())
        ...
        retrofitData.enqueue(object : Callback<List<PlayersItem>?> {
            override fun onResponse(call: Call<List<PlayersItem>?>, response: Response<List<PlayersItem>?>) {
                ...
                if (response.isSuccessful) {
                    ...
                    binding.raidersRv.adapter = playersAdapter
                }
                    else {
