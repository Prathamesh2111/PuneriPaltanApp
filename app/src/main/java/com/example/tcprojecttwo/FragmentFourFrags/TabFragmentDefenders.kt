...
import com.example.tcprojecttwo.databinding.FragmentTabRaidersBinding
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class TabFragmentDefenders : Fragment() {

    lateinit var playersAdapter: PlayersAdapter
    private var _binding: FragmentTabDefendersBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentTabDefendersBinding.inflate(inflater, container, false)

        binding.defendersRv.setHasFixedSize(false)
        linearLayoutManager = LinearLayoutManager(context)
        binding.defendersRv.layoutManager = linearLayoutManager
    ...
    fun getPlayersData() {
        val retrofitData = RetrofitInstance.apiInterface.getPlayers(2.toString())
        ...
        retrofitData.enqueue(object : Callback<List<PlayersItem>?> {
            override fun onResponse(call: Call<List<PlayersItem>?>, response: Response<List<PlayersItem>?>) {
                ...
                if (response.isSuccessful) {
                    ...
                    binding.defendersRv.adapter = playersAdapter
                }
                else {
